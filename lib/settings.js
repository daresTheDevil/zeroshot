...
function applyLegacyModelBounds(settings) {
  ...
  const anthropic = settings.providerSettings.anthropic || {};
  const legacyMaxLevel = mapLegacyModelToLevel(settings.maxModel);
  const legacyMinLevel = mapLegacyModelToLevel(settings.minModel);

  if (legacyMaxLevel) {
    anthropic.maxLevel = legacyMaxLevel;
  }

  if (legacyMinLevel) {
    anthropic.minLevel = legacyMinLevel;
  }

  const minRank = LEVEL_RANKS[anthropic.minLevel] || LEVEL_RANKS.level1;
  const maxRank = LEVEL_RANKS[anthropic.maxLevel] || LEVEL_RANKS.level3;
  const defaultRank = LEVEL_RANKS[anthropic.defaultLevel] || LEVEL_RANKS.level2;

  if (minRank > maxRank) {
    anthropic.minLevel = 'level1';
    anthropic.maxLevel = 'level3';
  } else if (defaultRank < minRank) {
    anthropic.defaultLevel = anthropic.minLevel;
  } else if (defaultRank > maxRank) {
    anthropic.defaultLevel = anthropic.maxLevel;
  }
